...
async function delegator() {
  try {
    ...
    const excelDataDailyEnergy = await getExcelReadyDailyEnergy(
      getDailyEnergies
    );
    const infuseSections = await infuseSection(
      "Inverter",
      excelDataDailyEnergy
    );
    ...
    console.error("Error in delegator function:", error);
    return false;
  }
}
