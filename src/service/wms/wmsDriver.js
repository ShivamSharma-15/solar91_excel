...
const { getExcelReadyWmsData } = require("./excelReadyWmsData");

async function wmsDriver(sites) {
  let finalData = [];
  for (let i = 0; i < sites.length; i++) {
    let wmsData = await getWmsBySiteModel(sites[i].site_id);
